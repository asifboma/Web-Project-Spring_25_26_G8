...
$selectedMembers = $projectModel->getProjectMemberIds($project_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($deadline) || empty($color_label)) {
        ...
        $updated = $projectModel->updateProject($project_id, $name, $description, $deadline, $color_label);

        if ($updated) {
            $projectModel->replaceProjectMembers($project_id, $members);
